...
从整本书的体系上看，可以把这条闭环明确写出来。Ch36 负责制度层面的合规框架与分级标准；P09 负责数据进入模型系统前的分类、脱敏、权限、隔离、审计与预检；Ch37 负责训练与协作阶段的隐私保护；Ch22 则负责多模态检索与应用能力承接。这样一来，整套体系就不再是孤立章节，而是一条从“数据治理”走向“模型治理”再走向“应用治理”的完整链路。图37-11展示了合规治理、隐私流水线、联邦训练与应用能力之间的闭环关系。
