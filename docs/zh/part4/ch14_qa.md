...
标注平台不是简单的"发任务工具"，而是一套质量控制系统。本章面向负责数据标注平台、QA 体系与运营治理的团队，阐述大模型数据标注与传统 CV/NLP 标注的差异，以及平台能力、流程能力与运营能力的边界。章节先讨论单轮问答、多轮对话、比较标注、审稿式标注的任务建模与工作流，包括样本分发、权限控制、进度监控与返工机制；随后建立 QA 一致性管理体系，涵盖预审、复审、仲裁、抽检、盲审的组合模式，以及一致性指标、黄金集与陷阱题的使用方法；进而讨论人效、质量、周期、成本四类运营指标与外包供应商治理；最后说明标注知识库、QA 手册与复盘库的沉淀，并衔接数据运营组织建设。
